...
def do_level0_processing(path_flightdata, path_radar, path_radiometer, radiometer_date, is_planet=False):
  ...
  ds_183_lev0 = xr.open_dataset(f"{path_radiometer}/183/{radiometer_date}.BRT.NC").pipe(radiometer)
  ds_11990_lev0 = xr.open_dataset(f"{path_radiometer}/11990/{radiometer_date}.BRT.NC").pipe(
      radiometer
  )
  ds_kv_lev0 = xr.open_dataset(f"{path_radiometer}/KV/{radiometer_date}.BRT.NC").pipe(radiometer)

  level0data = PostProcessedHAMPData(ds_flightdata, ds_radar_lev0, ds_183_lev0, ds_11990_lev0, ds_kv_lev0)
  return level0data

def do_level1_processing(level0data: PostProcessedHAMPData, is_planet=False) -> PostProcessedHAMPData:
  ...
  level1data["radar"] = level0data["radar"].pipe(filter_radar, level0data.flightdata[phi]).pipe(
      correct_radar_height,
...
def do_post_processing(path_flightdata, path_radar, path_radiometer, radiometer_date, is_planet=False):
  ''' do level0 and level1 processing '''

  level1data = do_level1_processing(do_level0_processing(path_flightdata,
                                                         path_radar,
                                                         path_radiometer,
                                                         radiometer_date,
                                                         is_planet=is_planet),
