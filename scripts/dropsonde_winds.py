...
def horizontal_wind_direction(eastward, northward):
    """returns bearing (from north)"""
    direction = np.arctan2(eastward, northward) * 180 / np.pi  # [radians -> degrees]
    # direction = np.where(direction < 0, direction + 360, direction) #[bearing from north]
    return direction
