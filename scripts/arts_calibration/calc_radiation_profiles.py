...
# %% setup folders
if not os.path.exists(f"arts_calibration/{cfg['flightname']}"):
    os.makedirs(f"arts_calibration/{cfg['flightname']}")
if not os.path.exists(f"arts_calibration/{cfg['flightname']}/plots"):
    os.makedirs(f"arts_calibration/{cfg['flightname']}/plots")

# %% loop over cloud free sondes
# loop over cloud free sondes
print(f"Running {cloud_free_idxs.size} dropsondes for {cfg['flightname']}")
for i, sonde_id in tqdm(enumerate(cloud_free_idxs)):
    # get profiles
    ...
    fig.savefig(f"arts_calibration/{cfg['flightname']}/plots/{sonde_id}.png")

# %% save results
TBs_arts.to_csv(f"arts_calibration/{cfg['flightname']}/TBs_arts.csv")
TBs_hamp.to_csv(f"arts_calibration/{cfg['flightname']}/TBs_hamp.csv")
